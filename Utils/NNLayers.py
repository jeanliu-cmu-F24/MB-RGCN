...
def defineParam(name, shape, dtype=tf.float32, reg=False, initializer='xavier', trainable=True):
	...
	:param name: 'UEmbed'/ 'IEmbed'
	:param shape: [args.user/args.item, args.latdim]
	:param dtype: tf.float32
	:param reg: True
	:param initializer:
	:param trainable:
	:return:
	'''
	#TODO what is params and what is regParams??
	global params
	global regParams
...
def FC(inp, outDim, name=None, useBias=False, activation=None, reg=False, useBN=False, dropout=None, initializer='xavier', reuse=False):
	...
	:param inp: tf.identity(temlat) - user * latent
	:param outDim: args.memosize
	:param name: None by default dfltP%d
	:param useBias: True/False
	:param activation: "relu"
	:param reg: True/False
	:param useBN: batch normalization
	:param dropout: dropout
	:param initializer:
	:param reuse:
	:return: dimensionality: user * memory size
	'''
	global params
	global regParams
	global leaky
	inDim = inp.get_shape()[1] # latent dimension
	temName = name if name!=None else 'defaultParamName%d'%getParamId()
	W = getOrDefineParam(temName, [inDim, outDim], reg=reg, initializer=initializer, reuse=reuse) # latent dimension * memory size
	if dropout != None:
...
def selfAttention(localReps, number, inpDim, numHeads):
	...
	:param localReps: list of [user/item * latent]
	:param number: # of behaviors
	:param inpDim: latent dimensions
	:param numHeads:
	:return: list of [user/item * latent]
	"""
	...
	# stack: [ user/item * behavior * latent]
	# reshape: [(user/item * behavior) * latent]
	rspReps = tf.reshape(tf.stack(localReps, axis=1), [-1, inpDim])
	# user/item * behavior * 1 * heads * latent/heads
	q = tf.reshape(rspReps @ Q, [-1, number, 1, numHeads, inpDim//numHeads])
	# user/item * 1 * behavior * heads * latent/heads
