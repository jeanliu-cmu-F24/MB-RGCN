...
def transToLsts(mat, mask=False):
	'''
	convert the 2-D matrix to a list (1D-np array), keep the index information in another list (np array)
	:param mat: matrix
	:param mask:
	:return: return index, data, shape
	index is a numpy array of [[row, col]] - dimension (n,2)
	data id a numpy array of data values - dimension (n, )
	shape a list of [row-dimension, col - dimension]
	'''
	...
	if mask:
		...
		data = data * spMask  # element wise multiplication
