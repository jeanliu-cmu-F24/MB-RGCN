...
class Recommender:
	...
	def defineModel(self):
		...
		uBehLats = [None] * args.behNum
		iBehLats = [None] * args.behNum
		for beh in range(args.behNum):
			ulats = [uEmbed0]
			ilats = [iEmbed0]
			for i in range(args.gnn_layer):
				ulat = self.messagePropagate(ilats[-1], self.adjs[beh])
				ilat = self.messagePropagate(ulats[-1], self.tpAdjs[beh])
				ulats.append(ulat + ulats[-1])
				ilats.append(ilat + ilats[-1])
			uBehLats[beh] = tf.concat(ulats, axis=-1)
			iBehLats[beh] = tf.concat(ilats, axis=-1)
		ulat = tf.add_n(uBehLats) / args.behNum
		ilat = tf.add_n(iBehLats) / args.behNum
