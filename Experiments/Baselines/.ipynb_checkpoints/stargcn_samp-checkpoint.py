...
class Recommender:
	...
	def messagePropagate(self, lats, adj, rank):
		hid = Activate(FC(tf.sparse.sparse_dense_matmul(adj, lats), rank, reg=True), self.actFunc)
		ret = Activate(FC(hid, args.latdim, reg=True), self.actFunc)
		reconLoss = tf.reduce_sum(tf.square(lats - ret))
		return ret, reconLoss
	...
	def defineModel(self):
		...
		for i in range(args.gnn_layer):
			ulat, recon = self.messagePropagate(ilats[-1], self.adjs, args.latdim//2)
			reconLoss += recon
			ilat, recon = self.messagePropagate(ulats[-1], self.tpAdjs, args.latdim//2)
			reconLoss += recon
	...
	def prepareModel(self):
		...
		self.adjs = tf.sparse_placeholder(dtype=tf.float32)
		self.tpAdjs = tf.sparse_placeholder(dtype=tf.float32)
	...
	def trainEpoch(self):
		...
		for s in range(bigSteps):
			...
			feed_dict[self.adjs] = transToLsts(pckAdjs[-1], norm=True)
			feed_dict[self.tpAdjs] = transToLsts(pckTpAdjs[-1], norm=True)
