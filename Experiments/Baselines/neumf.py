...
class Recommender:
	...
	def defineModel(self):
		...
		lat2 = tf.concat([pckulat, pckilat], axis=-1)

		lat11 = tf.squeeze(FC(lat1, 1, reg=True))
		lat21 = FC(lat2, args.latdim, useBias=True, reg=True, activation='relu')
		lat22 = tf.squeeze(FC(lat21, 1, reg=True))
