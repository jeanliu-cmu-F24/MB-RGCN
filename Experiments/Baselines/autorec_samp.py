...
class Recommender:
	...
	def prepareModel(self):
		...
		self.input = tf.placeholder(dtype=tf.float32, shape=[None, args.item], name='input')
		self.input = tf.sparse_placeholder(dtype=tf.float32)
		W = NNs.defineParam('W', [args.item, args.latdim], reg=True)
		V = NNs.defineParam('V', [args.item, args.latdim], reg=True)
		bias = NNs.defineParam('bias', [args.latdim])
		itmBias = NNs.defineParam('itmBias', [args.item])
		pckW = tf.nn.embedding_lookup(W, self.all_itms)
		pckV = tf.nn.embedding_lookup(V, self.all_itms)
		pckItmBias = tf.nn.embedding_lookup(itmBias, self.all_itms)

		uEmbed = Activate(tf.sparse.sparse_dense_matmul(self.input, pckW) + bias, 'sigmoid')
		iEmbed = pckV
		pckUEmbed = tf.nn.embedding_lookup(uEmbed, self.uids)
		pckIEmbed = tf.nn.embedding_lookup(iEmbed, self.iids)
		pckItmBias = tf.nn.embedding_lookup(pckItmBias, self.iids)
		self.preds = tf.reduce_sum(pckUEmbed * pckIEmbed, axis=-1) + pckItmBias
	...
	def trainEpoch(self):
		...
		for s in range(bigSteps):
			...
			feed_dict = {self.all_itms: itms}
			feed_dict[self.input] = transToLsts(pckAdjs[-1])
			...
			for i in range(steps):
				...
				uLocs, iLocs = self.sampleTrainBatch(batIds, pckAdjs[-1], len(itms))
