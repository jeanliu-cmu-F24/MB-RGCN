...
class Recommender:
	...
	def defineModel(self):
		...
		uEmbed = tf.nn.embedding_lookup(alluEmbed, self.all_usrs)
		iEmbed = tf.nn.embedding_lookup(alliEmbed, self.all_itms)
		pckulat = tf.nn.embedding_lookup(uEmbed, self.uids)
		pckilat = tf.nn.embedding_lookup(iEmbed, self.iids)

		lat1 = pckulat * pckilat

		preds = tf.squeeze(FC(lat1, 1, reg=True)) * args.mult
		return preds
